...
/*
	○ 위 ..\\Shaders\\
	 - hlsl 의 폴더 위치를 나타내기 위해 사용되는 Directory
	 - . 는 현재 폴더, .. 는 부모 폴더를 의미 (깃 문법과 같음)
	 - . 의 현재폴더란, 프로젝트 속성 -> 디버깅 -> 작업 디렉터리 에 입력된, $(ProjectionDir) 을 의미
	 - 따라서 위 코드의 의미는, $(ProjectionDir) 인 Engine 폴더의 부모인 $(SolutionDir) 로 간 후, Shaders 폴더
	   로 이동하고, 그 폴더 내에 있는 file (hlsl 파일들) 을 읽어오라는 의미

	○ 아래 내용들을 모두 공부하는 건 무리이고, 간단히 내용만 기억하면,
	 - 위 지정한 ..\\Shaders\\ + file 에 위치한 fx 파일을 읽어드려 Shading 을 처리.
	 - Dx 2D 때 만들었던 파이프라인 + Shader + Material 작업들을 모두 한번에 처리하는 코드
*/
{
